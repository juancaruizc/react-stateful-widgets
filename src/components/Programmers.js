...
export default function Programmers() {
  ...
  const [featuredProgrammers, setFeaturedProgrammers] = useState()
  ...
  const getNameOfFeatured = () => {
    ...
    const newArr = programmers.filter((dev) => dev.id === featuredProgrammers);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredProgrammers ? 'gold' :'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeaturedProgrammers(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
